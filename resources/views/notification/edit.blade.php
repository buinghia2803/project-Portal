        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.notification.update', [$notification->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('published_date') ? 'is-invalid' : '' }}" type="date"
                        name="published_date" id="published_date"
                        value="{{ old('published_date', $notification['published_date']) }}" required>
                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <select name="status" class="form-control" aria-label="Default select example"
                            ...
                            @foreach ($status as $key => $status)
                                <option value="{{ $key }}"
                                    {{ $notification->status == $key ? 'selected' : '' }}>
                                    {{ $status }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" aria-label="Default select example" name="published_to"
                        style="width: 200px; margin-right: 12px;">
                        <option>All</option>
                        @foreach ($divisions as $division)
                            <option value="{{ $division['id'] }}"
                                {{ $notification->published_to == $division['id'] ? 'selected' : '' }}>
